$(function () {
    ...
    var type3 = "人数";
    var type4 = "地区生产总值";
    var typename = '小学国家财政性教育经费占GDP比重';
    var typedw = '(%)'// /亿元
    var defaultSel = '已选省份';
    var min = 0;
    var max = 65000000;
        ...
        methods: {
            ...
            getZZToptions: function () {
                ...
                for (var i = 0; i < this.yearsnum; i++) {            //年份大循环
                    ...
                    for (var j=0;j<this.prosnum;j++){
                        for (var h=0;h<this.prosnum;h++){
                            if(this.studentdata[back]["data"][i][h]["name"]==pros[j]){
                                gdplist1.push(this.studentdata[back]["data"][i][h][type4])
                                gdplist2.push(this.studentdata[back2]["data"][i][h][type2])
                                blist[j]=(gdplist2[j]/gdplist1[j]*1000).toFixed(2)
                            }
                        }
                    ...
                    for (var h=0;h<this.prosnum;h++){
                        gdplist4.push(this.studentdata[back]["data"][i][h][type4]);
                        gdplist5.push(this.studentdata[back2]["data"][i][h][type2]);
                        blist2[h]=(gdplist5[h]/(gdplist4[h]*1000)).toFixed(2);
                        if(this.ps == 1){
                            vlist.push({
                    ...
                    for (var j = 0; j < this.prosnum; j++) {
                        ...
                        propotion[j]=this.studentdata[back2]["data"][i][j][type2]/(this.studentdata[back]["data"][i][j][type4]*100000);
                        gList.push([this.studentdata[back]["data"][i][j][type4]*100000000,this.studentdata[back2]["data"][i][j][type2]*1000,propotion[j]]);
                        //第一个是student，第二个是money，第三个是proption
                    }
                    giniarr.push(this.gini(gList));  //算基尼系数的
                    if(i==8){this.gini1(gList)}
                    var opt = {
