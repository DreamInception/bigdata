        ...
        methods: {
            ...
            requestAllData: function (url, seq) {
                if (seq == 1) {
                    ...
                    this.prosnum = this.studentdata[back]["data"][0].length;
                    this.yearsnum = this.studentdata[back]["time"].length;
                } else if (seq == 2) {
            ...
            initGini: function () {
                ...
                this.$nextTick(function () {
                    this.gcharts = echarts.init(document.getElementById(giniMap));
                    ...
                    window.onresize = function () {
                        if (resizeTimer) clearTimeout(resizeTimer);
                        resizeTimer = setTimeout(function () {
                            that.gcharts = echarts.init(document.getElementById(giniMap));
            ...
            getZZToptions: function () {
                ...
                for (var i = 0; i < this.yearsnum; i++) {            //年份大循环
                    ...
                    for (var j=0;j<this.prosnum;j++){
                        for (var h=0;h<this.prosnum;h++){
                            if(this.studentdata[back]["data"][i][h]["name"]==pros[j]){
                                gdplist1.push(this.studentdata[back3]["data"][i][h][type2])
                                gdplist2.push(this.studentdata[back3]["data"][i][h][type3])
                                blist[j]=(gdplist1[j]/gdplist2[j]*1000).toFixed(2)
                            }
                        }
                    ...
                    for (var h=0;h<this.prosnum;h++){
                        gdplist4.push(this.studentdata[back3]["data"][i][h][type2]);
                        gdplist5.push(this.studentdata[back3]["data"][i][h][type3]);
                        blist2[h]=(gdplist4[h]/(gdplist5[h]*1000)).toFixed(2);
                        vlist.push({
                    ...
                    for (var j = 0; j < this.prosnum; j++) {
                        if(back3=='xiaoxue'){
                            propotion[j]=this.studentdata[back3]["data"][i][j][type2]/(this.studentdata[back3]["data"][i][j][type3]);
                            gList.push([this.studentdata[back3]["data"][i][j][type3],this.studentdata[back3]["data"][i][j][type2],propotion[j]]);
                        }else if(back3=='chuzhong' || back3=='gaozhong'){
                            propotion[j]=this.studentdata[back3]["data"][i][j][type2]
                            propotion2[j]=propotion[j]/this.studentdata[back3]["data"][i][j][type3]
                            gList.push([this.studentdata[back3]["data"][i][j][type3],propotion[j],propotion2[j]])
                        }
                ...
                for (var i = 0; i < this.prosnum1; i++) {
                    if (this.rates[i]['value'] == 1) {
                        ...
                        for (var j = 0; j < this.yearsnum1; j++) {
                            var tmp = this.studentdata1[back3]['data'][j][i][typename];
                            tmp = tmp.toFixed(2);
                            if (this.studentdata1[back3]['data'][j][i][typename] == 0) {
                                tmp = null;
                            };
                            pdata.push(tmp);
                        }
